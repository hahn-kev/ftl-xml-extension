...
import {MusicPaths, SoundPaths} from '../data/autocomplete-value-sets';
...
export class FtlDataProvider implements IHTMLDataProvider {
  ...
  updateFtlData(root: FtlRoot) {
    SoundPaths.values.length = 0;
    SoundPaths.values.push(...root.soundFiles.map((file) => ({name: file.modPath})));
