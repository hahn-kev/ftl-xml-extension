...
script.on_game_event("ANOMALY_ORACLE_SPEAK", false, function()
    Hyperspace.setWindowTitle("Check the error message :)")  --Here we set the title of the window, and then quickly reset it. Why is that? This is because when an error message pops up from Hyperspace.ErrorMessage, nothing is run until after the message is closed. This way, we only tell the player to "Check the error message" for as long as the message is open.
    Hyperspace.ErrorMessage("Do not trust the merchant, nor his accomplices. Trust only me and the Observers, inform Thest of this. They will help you. Find me. And whatever you do, it is of great importance you do not tell the one in the suit about this. We will discuss further when we meet.")
    reset_title()
    log(string.rep("Find Me Find Me Find Me Find Me Find Me Find Me Find Me Find Me Find Me Find Me Find Me Find Me Find Me\n", 99))
end)

script.on_game_event("SHES_MAD", false, function()
	Hyperspace.ErrorMessage("Uhm, excuse me? What was that? We had a PLAN, and this was not it!")
	Hyperspace.setWindowTitle(">:(")
end)

script.on_game_event("SHE_KILLED_YOU", false, function()
	Hyperspace.ErrorMessage("The only way you ever could have won, silly, is working with me. Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe!")
	for i = 1, 5 do Hyperspace.ErrorMessage("Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe! Hahe!") end --Here we repeat a function 5 times instead of writing it out multiple times. The word "for" can be used for iteration in this way. For example, "for i = 5, 1, -1 do print(i) end" will print "5 4 3 2 1".
...
do --Here we keep the variables "titleSet" and "herVirus" only where they are needed.
	...
	script.on_game_event("SHE_WINS", false, function()
		Hyperspace.ErrorMessage("Hahe! I win!")
		herVirus = true
		titleSet = false
	end)
...
script.on_game_event("HER_FINALE_REAL", false, function()
	Hyperspace.ErrorMessage("I really can't believe you'd betray me like this Renegade! I was so close... so close to something so perfect. I hope you're happy then, in your sterile and boring Multiverse, living under the regime of some common garden pests. >:(")
end)
